...
- **Prisma** + **SQLite / Postgres**（本地默认 SQLite；Vercel 使用 Supabase Postgres 持久化）
...
完整部署步骤、Vercel 发布、生产启动、Postgres 切换和故障排查见 [部署手册](./docs/DEPLOYMENT.md)。
...
### Vercel / Supabase Postgres 部署

项目保留两份 Prisma schema：

- [prisma/schema.prisma](./prisma/schema.prisma)：本地 SQLite 开发默认使用。
- [prisma/schema.postgres.prisma](./prisma/schema.postgres.prisma)：Vercel / Supabase Postgres 部署使用。

`postinstall` 会根据 `DATABASE_URL` 自动选择生成 SQLite 或 Postgres Prisma Client；如果 Vercel/Supabase 注入的是 `POSTGRES_PRISMA_URL`、`POSTGRES_URL_NON_POOLING` 或 `POSTGRES_URL`，脚本也会自动识别。Vercel 构建通过 `npm run vercel:build` 执行 `prisma db push`、内置剧本 seed 与 `next build`。线上环境需要配置：
